...
with st.expander("✏️ Datenpunkte verwalten", expanded=_mgmt_expanded):
    ...
    # Statusmeldungen außerhalb der Tabs — immer sichtbar unabhängig vom aktiven Tab
    if "mgmt_msg" in st.session_state:
        lvl, txt = st.session_state.pop("mgmt_msg")
    ...
    with mgmt_new:
        ...
        # Außerhalb des Form-Kontexts
        if submitted:
            kat = inp_kat_neu.strip() if inp_kat_sel == "✏️ Neue Kategorie…" else inp_kat_sel
            if not kat:
                st.session_state["mgmt_msg"] = ("err", "Bitte eine Kategorie angeben.")
            else:
                try:
                    append_transaction(
                    ...
                    st.session_state["mgmt_msg"] = ("ok", msg)
                except Exception as ex:
                    st.session_state["mgmt_msg"] = ("err", f"Fehler beim Speichern: {ex}")
    ...
    with mgmt_edit:
        ...
        if save_ok:
            try:
                ...
                st.cache_data.clear()
                st.session_state["mgmt_msg"] = ("ok", "✓ Transaktion aktualisiert.")
            except Exception as ex:
                st.session_state["mgmt_msg"] = ("err", f"Fehler: {ex}")
            st.session_state["_mgmt_open"] = True
            st.rerun()

        if del_ok:
            if not confirm_del:
                st.session_state["mgmt_msg"] = ("err", "Bitte Löschen per Checkbox bestätigen.")
            else:
                try:
                    delete_transaction(sel_sr)
                    st.cache_data.clear()
                    st.session_state["mgmt_msg"] = ("ok", "✓ Transaktion gelöscht.")
                except Exception as ex:
                    st.session_state["mgmt_msg"] = ("err", f"Fehler: {ex}")
            st.session_state["_mgmt_open"] = True
            st.rerun()
