...
def prepare(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Deutsches Zahlenformat: Punkt = Tausendertrennzeichen, Komma = Dezimaltrennzeichen
    series = df[c["col_amount"]].astype(str).str.strip()
    german = series.str.contains(",", regex=False)
    series = series.where(~german, series.str.replace(".", "", regex=False))
    series = series.str.replace(",", ".", regex=False)
    series = series.str.replace(r"[^\d.\-]", "", regex=True)
    df[c["col_amount"]] = pd.to_numeric(series, errors="coerce")
